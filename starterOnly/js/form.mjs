...
class Form extends Mediator {
    ...
    isValid() {
        ...
        const tournamentsIsValid = Validator.positiveInteger(this.data.tournaments);
    ...
    handleNameChange = ([target, value]) => {
        target.setState();
        if (value) {
            target.setState(Validator.name(value) ? 'valid' : 'invalid');
        }
        switch (target) {
            case this.firstData:
    ...
    handleEmailChange = ([target, value]) => {
        target.setState();
        if (value) {
            target.setState(Validator.email(value) ? 'valid' : 'invalid');
        }
    ...
    handleTournamentsChange = ([target, value]) => {
        target.setState(Validator.positiveInteger(value) ? 'valid' : 'invalid');
        this.data.tournaments = value;
    }
